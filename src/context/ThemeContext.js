...
const initialState = {
  ...
  windowWidth: 0,
  windowHeight: 0,
  loginMode: document.body.dataset.login || "front", // Default to 'front'
...
const { 
  ...
  const toggleTheme = () => {
    const newTheme = background.value === "dark" ? "light" : "dark";
    changeBackground({ value: newTheme, label: newTheme.charAt(0).toUpperCase() + newTheme.slice(1) });
  };

  
  // Update body attribute when loginMode changes
  useEffect(() => {
    document.body.dataset.login = state.loginMode;
  }, [state.loginMode]);

  // Function to update login mode
  const setLoginMode = (mode) => {
    dispatch({ loginMode: mode });
  };
